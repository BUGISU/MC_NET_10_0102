...
## 📚 적용 교과 단원 및 콘텐츠 연계 (2022 개정 교육과정 기준)

| 단원 코드          | 성취 기준                         | 콘텐츠 적용 예시 |
| -------------- | ----------------------------- | --------- |
| **\[4과04-02]** | 물질이 열을 받거나 잃을 때 상태가 변함을 관찰한다. |           |

* `ExperimentalController`를 통해 얼음이 열을 받아 물로, 물이 가열되어 기체로 변화하는 과정을 애니메이션과 파티클로 시각화
* 상태 변화 실험 UI를 통해 **직접 가열/냉각을 실행**하며 변화 과정을 확인&#x20;
  |
  \| **\[4과04-03]** | 고체, 액체, 기체의 상태에 따라 성질이 다름을 비교한다. |
* `MassVolumeController`를 통해 각 상태(얼음, 물, 수증기)의 **부피 차이와 성질 차이 설명 제공**
* 버튼 클릭으로 실험 진행 → InfoText 및 팝업으로 성질 비교 설명 출력&#x20;
  |
  \| **\[4과04-04]** | 물의 상태 변화가 물의 순환과 관련 있음을 이해한다. |
* `WaterCycle_UIManager`에서 증발, 응결, 강수, 모임 과정을 카메라 이동과 팝업으로 체험
* 순서대로 카메라가 이동하며 자연음과 함께 **물의 순환 흐름을 시각적으로 안내**&#x20;
  |
  \| **\[4과04-05]** | 일상생활 속에서 물질의 상태 변화를 찾아 설명한다. |
* `QuizSettingController`에서 일상에서 접할 수 있는 상태 변화 문제 예시 제공 (예: 냉장고 병 물방울, 얼음이 부피 증가로 컵 파손 등)
* 각 보기마다 정답/오답 피드백 및 설명 멘트 포함&#x20;
  |
